...
class TestAngle:
    ...
    @pytest.mark.parametrize(
        "init_val, init_unit, target_unit, expected",
        [
...
class TestAngleMath:
    ...
    def test_angle_angle_math(self, angle1_val, angle1_unit, angle2_val, angle2_unit):
        ...
        assert np.allclose(Angle(angle1.internal_value + angle2.internal_value), angle_add)

        angle_sub = angle1 - angle2
        assert isinstance(angle_add, Angle | AngleArray)
        assert angle_add.display_unit == angle1_unit
        assert np.allclose(Angle(angle1.internal_value - angle2.internal_value), angle_sub)
