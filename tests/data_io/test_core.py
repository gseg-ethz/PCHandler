...
from pchandler.data_io import find_point_cloud_in_directory
...
from pchandler.scalar_fields import NormalFields, RGBFields
...
class TestAbstractIOMethods:
    ...
    def test_extract_scalar_triplets(self, struct_array):
        ...
        normals = AbstractIOHandler._extract_scalar_field_triplet(
            struct_array, N, normal_fields, NormalFields, field_names
        )
    ...
    def test_generate_struct_array(self):
        for i in range(2):
            ...
            struct_arr = AbstractIOHandler._generate_structured_array(
                pcd, scalar_fields=["rf", "gf", "bf", "sf1"], add_prefix=False, prefix="", revert_sf_types=False
            )
